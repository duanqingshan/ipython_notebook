...
TTR/TTR.py at master · Quantmatic/TTR · GitHub  
https://github.com/Quantmatic/TTR/blob/master/TTR.py
"""
...
def cyc(ohlcv, periods=61):
    u'''
    陈浩-杨新宇成本均线
    '''
    vol = ohlcv.volume
    tmp = vol[:periods-1].cumsum()
    vol_sum = vol.rolling(periods).sum().fillna(0)
    #vol_sum = vol_sum.fillna(method='bfill')
    #vol_sum = vol_sum + tmp
    vol_sum = vol_sum.add(tmp, fill_value=0) 
    # better than above, with support of fill_value
    factor = vol / vol_sum
    o,h,l,c = ohlcv.open, ohlcv.high, ohlcv.low, ohlcv.close
    price = (o+3*h+l+2*c)/7
    average = dma(price, factor)
    return pd.Series(average, index=c.index, name='DMA_%d'%periods)
